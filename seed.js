...
let addMedias = async (arr, user) => {
  ...
  let mediaOwner = await User.findOne({ username: thisUser.username });
  let mediaData = thisUser.latestMedia.map(async m => {
    let mediaParams = {
      caption: m.caption,
      likes: m.likes,
      commentCount: m.commentsCount
    }
    let media = await new Media(mediaParams).save()
    let updatedMedia = await Media.findOneAndUpdate({ _id: media._id }, { $set: { user: mediaOwner } }, { new: true })
    return updatedMedia
  })
